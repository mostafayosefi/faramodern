...
@foreach($cats as $cat)
 <option @if($cat->cat_id==$admin->pro_cat) selected="" @endif value="{{$cat->cat_id}}">{{$cat->cat_name}}</option>
@endforeach
...
<input type="text" class="form-control" id="exampleInputUsername1" autocomplete="off" placeholder="نام محصول" name="name"  value="{{$admin->pro_name}}"  required > 
</div>	
 ...
 <textarea class="form-control" name="short"  rows="6" required>{{$admin->pro_kh}}</textarea>
 </div>

 <div class="form-group">	
 <h4 class="card-title"> توضیحات کامل محصول</h4>
 <textarea class="form-control" name="des" id="tinymceExample" rows="10"  >{{$admin->pro_des}}</textarea>
 </div>
			
<hr> 
<img src="{{env('APP_URL')}}/public/images/{{$admin->pro_img}}" height="200px"  width="200px" />
<hr> 
...
<table width="100%" id="multi_file_uploader" class="table table-bordered dt-responsive nowrap" >
 ...
 <td>{{$i++}}</td>
 <td valign="top" align="right">
<img class="profile-pic" src="{{env('APP_URL')}}/public/images/{{$file->file_name}}"  />
</td>
 <td valign="top"> 
 <a href="{{env('APP_URL')}}/superadmin/viewsprods/{{$admin->pro_id}}/deletfile/{{$file->file_id}}">
<span class="btn btn-danger btn-sm" >حذف</span></a>
 </td>
 <td>
 <div class="form-check">
 <label class="form-check-label">
 <a href="{{env('APP_URL')}}/superadmin/viewsprods/{{$admin->pro_id}}/accfile/{{$file->file_id}}">
 <input type="radio" class="form-check-input" name="optionsRadios" id="optionsRadios" value="option1"  @if($file->file_name==$admin->pro_img)checked="" @endif >
  @if($file->file_name==$admin->pro_img)<span class="badge badge-success">تصویر پیش فرض<i data-feather="check-circle"></i></span> @else انتخاب به عنوان تصویر اصلی @endif
...
<input type="number" class="form-control" id="exampleInputUsername1" autocomplete="off" placeholder="به ریال" name="price"  required   value="{{$admin->pro_price}}" > 
</div>	
...
<input type="text" class="form-control" id="exampleInputUsername1"   pattern="\d+" autocomplete="off" placeholder="به درصد" name="ind_bis"  value="{{$admin->pro_bis}}"  required > 
 </div>
